...
lowerAndGreater :: (Ord a, Show a) => a -> [a] -> String
lowerAndGreater x xs =
  let lowerCount   = length $ filter (< x) xs
      greaterCount = length $ filter (> x) xs
  in show x ++ " is greater than " ++ show lowerCount ++ " elements and lower than " ++ show greaterCount ++ " elements"
